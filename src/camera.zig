...
pub fn applyTentFilter(samples: *[config.NUM_SAMPLES_PER_PIXEL * config.NUM_SCREEN_DIMS]f64) void {
    ...
    while (sample_idx < config.NUM_SAMPLES_PER_PIXEL) : (sample_idx += 1) {
        x = samples[sample_idx * config.NUM_SCREEN_DIMS] * @as(f64, config.NUM_SCREEN_DIMS);
        y = samples[sample_idx * config.NUM_SCREEN_DIMS + 1] * @as(f64, config.NUM_SCREEN_DIMS);
        samples[sample_idx * config.NUM_SCREEN_DIMS] = if (x < 1.0) @sqrt(x) - 1.0 else 1.0 - @sqrt(2.0 - x);
        samples[sample_idx * config.NUM_SCREEN_DIMS + 1] = if (y < 1.0) @sqrt(y) - 1.0 else 1.0 - @sqrt(2.0 - y);
    }
}
