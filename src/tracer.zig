...
pub fn tracePaths(tracer: Tracer, pixels: []u8, offset: usize, size: usize) std.os.GetRandomError!void {
    ...
    const ray_factor = @splat(config.VECTOR_LEN, @as(f64, 136.0));
    ...
    while (pixel_offset < end_offset) : (pixel_offset += config.VECTOR_LEN) {
        ...
        while (anti_aliasing_factor < config.ANTI_ALIASING_FACTOR) : (anti_aliasing_factor += 1) {
            ...
            while (sample_idx < config.NUM_SAMPLES_PER_PIXEL) : (sample_idx += 1) {
                x_chunk_direction = x_direction * @splat(config.VECTOR_LEN, (((@intToFloat(f64, (anti_aliasing_factor & 1)) + 0.5 + chunk_samples[sample_idx * config.NUM_SCREEN_DIMS]) / 2.0) + @intToFloat(f64, x)) / screen_side - 0.5);
                y_chunk_direction = y_direction * @splat(config.VECTOR_LEN, -((((@intToFloat(f64, (anti_aliasing_factor >> 1)) + 0.5 + chunk_samples[sample_idx * config.NUM_SCREEN_DIMS + 1]) / 2.0) + @intToFloat(f64, y)) / screen_side - 0.5));
                ...
                raw_color += ray_color * @splat(config.VECTOR_LEN, 1.0 / @as(f64, config.NUM_SAMPLES_PER_PIXEL));
            }
