...
pub const Scene = struct {
    ...
    pub fn intersect(self: *Scene, cur_ray: ray.Ray) Hit {
        ...
        var hit = Hit{};
        for (self.spheres.items, 0..) |cur_sphere, i| {
    ...
    pub fn collectLights(self: *Scene) std.mem.Allocator.Error!void {
        for (self.spheres.items, 0..) |cur_sphere, light_idx| {
...
pub fn sampleLights(scene: *Scene, hit_point: vector.Vec4, normal: vector.Vec4, ray_direction: vector.Vec4, cur_material: *const material.Material) vector.Vec4 {
    ...
    var distance_to_light_sq: f64 = undefined;
    var sin_alpha_max_sq: f64 = undefined;
    ...
    for (scene.lights.items) |light_idx| {
        ...
        distance_to_light_sq = vector.dotProduct(hit_to_light_center, hit_to_light_center);
        ...
        if (shadow_ray_hit.sphere_idx_opt) |shadow_idx| {
            if (shadow_idx == light_idx) {
                if (cos_theta > 0.0) {
                    sin_alpha_max_sq = light.radius * light.radius / distance_to_light_sq;
                    cos_alpha_max = @sqrt(1.0 - sin_alpha_max_sq);
                    omega = 2.0 * (1.0 - cos_alpha_max);
                    cos_theta *= omega;
                    color += cur_material.diffuse * light.material.emissive * @as(vector.Vec4, @splat(cos_theta));
                }
                if (cur_material.material_type == material.MaterialType.GLOSSY or cur_material.material_type == material.MaterialType.MIRROR) {
                    ...
                    if (cos_theta > 0.0) color += cur_material.specular * @as(vector.Vec4, @splat(std.math.pow(f64, cos_theta, cur_material.specular_exponent)));
                }
            }
        }
    }
